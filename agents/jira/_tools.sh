...
add_worklog() {
    ...
    # Build JSON payload with strict formatting
...
EOF
    ...
    if ! echo "$json_payload" | jq . > /dev/null 2>&1; then
        echo "❌ Invalid JSON payload. Exiting." >&2
        return 1
    fi
    ...
    if [[ "$http_code" -eq 201 ]]; then
        ...
        echo "❌ Failed to add worklog. HTTP Status: $http_code" >&2
        echo "Response: $response_body" >> "$LLM_OUTPUT"
        return 1
    fi
}
